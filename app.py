...
fig1 = px.bar(top10, x='Country', y='Happiness Score', color='Region',
              title='Top 10 Happiest Countries (2016)', text_auto=True)

corr = df[num_cols].corr()
fig2 = px.imshow(corr, text_auto=True, color_continuous_scale='YlGnBu',
                 title='Correlation between Happiness Factors (2016)')

fig3 = px.scatter(df, x='Economy (GDP per Capita)', y='Happiness Score',
                  color='Region', hover_name='Country',
                  size='Health (Life Expectancy)', size_max=15,
                  title='GDP per Capita vs Happiness Score by Region')

region_avg = df.groupby('Region')['Happiness Score'].mean().reset_index()
fig4 = px.pie(region_avg, values='Happiness Score', names='Region',
              title='Average Happiness Score by Region', hole=0.3)

fig5 = px.scatter_geo(df, locations='Country', locationmode='country names',
                      color='Economy (GDP per Capita)', hover_name='Country',
                      size='Health (Life Expectancy)', projection='natural earth',
                      title='Global GDP & Life Expectancy (2016)',
                      color_continuous_scale='Viridis')

# ---------------------------------
# 3. Build Dashboard using Dash
# ---------------------------------
app = Dash(__name__)
...
app.layout = html.Div([
    ...
    html.P("Explore how economy, health, and freedom relate to happiness across regions.",
